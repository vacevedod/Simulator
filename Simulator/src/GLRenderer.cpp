...
{
    if (viewIndex >= m_occlusionMeshBuffers.size()) {
        return;
    }
    ...
    glDrawArrays(GL_TRIANGLES, 0, m_occlusionMeshVertexes);
...
void GLRenderer::queueObjects(const std::vector<ObjectRenderData>& matrices)
{
    if (m_instanceBuffer.queued > 0) {
        dispatchDraws();
    }

    // Map instance buffer
    if (m_instanceBuffer.data == nullptr) {
        m_instanceBuffer.data = reinterpret_cast<ObjectRenderData*>(glMapNamedBufferRange(
            m_perFrameBuffers[m_currentFrameBuffer].instanceBuffer, 0, sizeof(ObjectRenderData) * m_instanceBuffer.maxInstances, GL_MAP_WRITE_BIT));
    ...
    memcpy(m_instanceBuffer.data, matrices.data(), sizeof(ObjectRenderData) * matrices.size());
    m_instanceBuffer.queued = static_cast<int32_t>(matrices.size());
}

void GLRenderer::dispatchDraws()
{
    if (m_instanceBuffer.queued == 0) {
        return;
    }

    glUnmapNamedBuffer(m_perFrameBuffers[m_currentFrameBuffer].instanceBuffer);
    ...
    glDrawElementsInstanced(GL_TRIANGLES, m_currentGeometry->indexCount(), GL_UNSIGNED_INT, nullptr, m_instanceBuffer.queued);
    m_instanceBuffer.queued = 0;
    m_instanceBuffer.data = nullptr;
}
...
{
    for (int i = 0; i < 4 * 4; ++i) {
        GLuint instanceBuffer = createInstanceBuffer();
...
GLuint GLRenderer::createInstanceBuffer()
{
...
void GLRenderer::useInstanceBuffer()
{
    // Set up vertex attributes for instancing.
    glBindBuffer(GL_ARRAY_BUFFER, m_perFrameBuffers[m_currentFrameBuffer].instanceBuffer);

    for (int i = 2; i <= 9; i++) {
        glEnableVertexAttribArray(i);
        glVertexAttribPointer(i, 4, GL_FLOAT, false, sizeof(ObjectRenderData), reinterpret_cast<void*>(static_cast<uintptr_t>(sizeof(float) * (i - 2) * 4)));
...
void GLRenderer::createDummyWindow()
{
    const int width = 800;
    const int height = 600;

    const wchar_t* windowClassName = L"Varjo";
    const wchar_t* windowTitle = L"Varjo";
    ...
    if (!RegisterClassW(&windowClass)) {
        printf("RegisterClassEx failed: %s\n", getLastErrorString());
        abort();
    }

    m_hwnd = CreateWindowW(windowClassName, windowTitle, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width, height, NULL, NULL, hInstance, NULL);
    if (!m_hwnd) {
...
void GLRenderer::createOcclusionMesh()
{
    if (m_settings.useOcclusionMesh()) {
        glGenBuffers(static_cast<GLsizei>(m_occlusionMeshBuffers.size()), m_occlusionMeshBuffers.data());
        for (uint32_t i = 0; i < 2; i++) {
            varjo_Mesh2Df* occlusionMesh = varjo_CreateOcclusionMesh(m_session, i, varjo_WindingOrder_CounterClockwise);
            if (occlusionMesh->vertexCount == 0) {
                continue;
            }
            glBindBuffer(GL_ARRAY_BUFFER, m_occlusionMeshBuffers[i]);
            m_occlusionMeshVertexes = occlusionMesh->vertexCount;
